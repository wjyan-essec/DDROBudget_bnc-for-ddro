...
rem Usage: test_n50_incumbents_BC.bat [total time limit in seconds]
rem Re-tests with BC the five N50 instances that produced incumbents previously.
set "ROOT=%~dp0"
...
set "RUN_DIR=%ROOT%test_runs\N50_INCUMBENTS_BC_%RANDOM%_%RANDOM%"
if exist "%RUN_DIR%" (
...
"%PYTHON_EXE%" -c "import csv,sys; rows=list(csv.DictReader(open(sys.argv[1],newline='',encoding='utf-8'))); names=[r['instance'] for r in rows]; print('Completed rows:',len(rows),'/5'); sys.exit(0 if len(rows)==5 and len(set(names))==5 and all(r['method']=='intersection' for r in rows) else 1)" "%RUN_DIR%\summary.csv"
if errorlevel 1 set "FAILED=1"
...
echo All 5 selected N50 BC runs completed without an invalid high incumbent.
exit /b 0
