...
rem Usage: test_n10_intersection_all.bat [time limit in seconds]
rem Example: test_n10_intersection_all.bat 60
...
set "RUN_DIR=%ROOT%test_runs\N10_INTERSECTION_%RANDOM%_%RANDOM%"
if exist "%RUN_DIR%" (
...
echo All 10 intersection-cut results match the reference.
exit /b 0
...
echo Running N10 replicate %~1 with intersection cuts...
"%PYTHON_EXE%" "%ROOT%src\run_bnc.py" --instance_file "%INSTANCE%" --instance_type bobilib --lower_level general --cuts intersection --projected 1 --separation integer --time_lim %TIME_LIMIT% --verbose_level 0 --output_csv "%RUN_DIR%\summary.csv" >"%RUN_DIR%\%STEM%%~1_intersection.log" 2>&1
if errorlevel 1 (
    echo Solve failed for replicate %~1. See "%RUN_DIR%\%STEM%%~1_intersection.log".
    exit /b 1
)
